...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@finapticoos/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@finapticoos/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@finapticoos/adapter-utils").TranscriptEntry[];
