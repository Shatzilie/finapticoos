  ...
  const packageJson = {
    ...
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@finapticoos/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@finapticoos/plugin-sdk": sdkDependency,
...
import { createPluginBundlerPresets } from "@finapticoos/plugin-sdk/bundlers";
...
`,
  ...
  if (template === "environment") {
    writeFile(
      ...
      `import type { PaperclipPluginManifestV1 } from "@finapticoos/plugin-sdk";
...
`,
    ...
    writeFile(
      ...
      `import { definePlugin, runWorker } from "@finapticoos/plugin-sdk";
import type {
...
`,
    ...
    writeFile(
      ...
      `import { usePluginData, type PluginWidgetProps } from "@finapticoos/plugin-sdk/ui";
...
} from "@finapticoos/plugin-sdk/testing";
...
`,
    ...
    writeFile(
      path.join(outputDir, "src", "ui", "index.tsx"),
      `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@finapticoos/plugin-sdk/ui";
...
`,
    ...
    writeFile(
      path.join(outputDir, "tests", "plugin.spec.ts"),
      `import { describe, expect, it } from "vitest";
import { createTestHarness } from "@finapticoos/plugin-sdk/testing";
import manifest from "../src/manifest.js";
import plugin from "../src/worker.js";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@finapticoos/plugin-sdk\` and \`@finapticoos/shared\` from a local Paperclip checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.paperclip-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
- \`pnpm build\` uses esbuild presets from \`@finapticoos/plugin-sdk/bundlers\`.
