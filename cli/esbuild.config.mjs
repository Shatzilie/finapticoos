/**
 * esbuild configuration for building the FinapticoOS CLI for npm.
 *
 * Bundles all workspace packages (@finapticoos/*) into a single file.
...
// to npm and resolved at runtime (e.g. @finapticoos/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@finapticoos/")) {
      externals.add(name);
    }
  }
