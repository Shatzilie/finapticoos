...
async function runCliJson<T>(
  ...
  const cliArgs = ["--silent", "@finapticoos/cli", ...args];
  if (opts.apiBase) {
...
async function waitForServer(
  ...
  while (Date.now() - startedAt < 30_000) {
    if (child.exitCode !== null) {
      throw new Error(
        `finapticoos run exited before healthcheck succeeded.\nstdout:\n${output.stdout.join("")}\nstderr:\n${output.stderr.join("")}`,
      );
    }
...
describeEmbeddedPostgres("finapticoos company import/export e2e", () => {
  let tempRoot = "";
  let configPath = "";
  let exportDir = "";
  let apiBase = "";
  ...
  beforeAll(async () => {
    ...
    const child = spawn(
      "pnpm",
      ["@finapticoos/cli", "run", "--config", configPath],
      {
        cwd: repoRoot,
