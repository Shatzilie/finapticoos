...
if (worktreeEnvBootstrap.missingEnv) {
  console.error(
    `[paperclip] linked git worktree at ${repoRoot} is missing ${path.relative(repoRoot, worktreeEnvBootstrap.envPath)}. Run \`finapticoos worktree init\` in this worktree before \`pnpm dev\`.`,
  );
  process.exit(1);
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@finapticoos/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[paperclip] Command failed with code ${status.code}: pnpm --filter @finapticoos/db exec tsx src/migration-status.ts --json\n`,
    );
...
async function buildPluginSdk() {
  ...
  const result = await runPnpm(
    ["--filter", "@finapticoos/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@finapticoos/server", serverScript, ...forwardedArgs],
