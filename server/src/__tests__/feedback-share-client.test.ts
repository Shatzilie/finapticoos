...
describe("feedback trace share client", () => {
  ...
  it("throws when no backend url is configured (FinapticoOS: no default upstream)", async () => {
    ...
    ).rejects.toThrow(/Feedback trace share is disabled/);
  ...
  it("wraps the feedback trace payload as gzip+base64 json before upload", async () => {
    ...
    expect(call?.[0]).toBe("https://telemetry.example.com/feedback-traces");
