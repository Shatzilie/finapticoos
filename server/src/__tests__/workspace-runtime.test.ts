...
import type { WorkspaceOperation } from "@finapticoos/shared";
...
describe("ensureServerWorkspaceLinksCurrent", () => {
  it("relinks stale server workspace dependencies inside the current repo root", async () => {
    ...
    const serverNodeModulesScopeDir = path.join(repoRoot, "server", "node_modules", "@finapticoos");
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "@finapticoos/server",
        dependencies: {
          "@finapticoos/db": "workspace:*",
        },
      }),
      "utf8",
    );
    await fs.writeFile(
      ...
      JSON.stringify({ name: "@finapticoos/db" }),
      "utf8",
    );
    await fs.writeFile(
  ...
  it("fails instead of writing an unseeded fallback config when worktree init errors after CLI detection succeeds", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        ...
        [
          "#!/bin/sh",
          "if [ \"$1\" = \"finapticoos\" ] && [ \"$2\" = \"--help\" ]; then",
          "  exit 0",
          "fi",
          "if [ \"$1\" = \"finapticoos\" ] && [ \"$2\" = \"worktree\" ] && [ \"$3\" = \"init\" ]; then",
