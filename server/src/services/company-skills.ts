...
import { companies, companySkills } from "@finapticoos/db";
import { readPaperclipSkillSyncPreference } from "@finapticoos/adapter-utils/server-utils";
import type { PaperclipSkillEntry } from "@finapticoos/adapter-utils/server-utils";
import type {
  CompanySkill,
...
import { normalizeAgentUrlKey } from "@finapticoos/shared";
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("Shatzilie/finapticoos/")) return slug;
...
function deriveCanonicalSkillKey(
  ...
  if (sourceKind === "paperclip_bundled") {
    return `Shatzilie/finapticoos/${slug}`;
  }
